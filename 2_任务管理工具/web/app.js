...
function load(){
  try{const saved=JSON.parse(localStorage.getItem(DB_KEY));if(saved){saved.items=saved.items||{};for(const type of ['year','quarter','month','week','today'])saved.items[type]=(saved.items[type]||[]).map(x=>({...x,type:x.type||type,pinned:x.pinned===true}));saved.habits=(saved.habits||[]).map(h=>({...h,enabled:h.enabled!==false,startDate:h.startDate||today,history:Array.isArray(h.history)?h.history:[]}));saved.config=saved.config||[];localStorage.setItem(DB_KEY,JSON.stringify(saved));return saved}}catch{}
  ...
  return {items:{
    ...
    today:old.length?old.map(x=>({...x,type:'today',pinned:false})):[item('完成今天最重要的工作','','today','doing',today),item('阅读 20 分钟','','today','todo',today),item('整理桌面与收件箱','','today','done',today)]
...
function item(title,note,type,status,date){return{id:uid(),title,note,type,status,date,pinned:false,createdAt:Date.now()}}
...
function activeHabits(){return data.habits.filter(h=>h.enabled!==false)}
...
function pinnedWidgetSection(list){const content=list.map(x=>`<div class="mini-item pinned" data-id="${x.id}" data-type="${x.type}"><button data-toggle-item>✓</button><strong><i class="mini-pin">⌖</i>${esc(x.title)}</strong><span>${labels[x.type]} · ${fmtDate(x.date)}</span></div>`).join('');return `<section class="widget-section pinned-widget-section"><div class="widget-section-head"><i>⌖</i><strong>常驻任务</strong><small>${list.length} 项固定展示</small></div><div class="widget-section-body">${content}</div></section>`}
function widgetSection(cfg,index){
  const type=cfg.id,isHabit=type==='habits';let content='';let count=0;
  if(isHabit){const habits=activeHabits();count=habits.filter(h=>h.history.includes(today)).length;content=habits.map(h=>`<button class="mini-habit ${h.history.includes(today)?'checked':''}" data-habit="${h.id}"><b>${h.emoji}</b><strong>${esc(h.title)}</strong><span>${h.history.includes(today)?'今日完成':'轻触打卡'}</span></button>`).join('')}
  else{const list=(data.items[type]||[]).filter(x=>!x.pinned);count=list.filter(x=>x.status!=='done').length;content=list.slice(0,4).map(x=>`<div class="mini-item ${x.status==='done'?'done':''}" data-id="${x.id}" data-type="${type}"><button data-toggle-item>✓</button><strong>${esc(x.title)}</strong><span>${fmtDate(x.date)}</span></div>`).join('')}
  if(!content)content='<div class="widget-empty">这里还很轻，去深度管理添加一项吧</div>';
  return `<section class="widget-section ${cfg.collapsed?'collapsed':''}" style="animation-delay:${index*.06}s"><button class="widget-section-head" data-section="${type}"><i>${icons[type]}</i><strong>${labels[type]}</strong><small>${isHabit?`${count}/${activeHabits().length}`:`${count} 项进行中`}</small><em class="chevron">⌄</em></button><div class="widget-section-body">${content}</div></section>`;
}
...
function taskRow(x){return `<div class="task-row ${x.status==='done'?'done':''} ${x.pinned?'pinned':''}" data-id="${x.id}" data-type="${x.type}"><button class="check" data-toggle-item>✓</button><div><h4>${x.pinned?'<i class="task-pin">⌖ 常驻</i>':''}${esc(x.title)}</h4>${x.note?`<p>${esc(x.note)}</p>`:''}</div><time>${fmtDate(x.date)}</time></div>`}
...
function goalCard(x,i){const p=x.status==='done'?100:x.status==='doing'?55:12;return `<article class="goal-card ${x.pinned?'pinned-task':''}" style="animation-delay:${i*.05}s" data-id="${x.id}" data-type="${x.type}"><div class="goal-top"><div><span class="goal-type">${x.status==='done'?'已完成':x.status==='doing'?'正在推进':'待开始'}</span>${x.pinned?'<span class="pin-badge">⌖ 桌面常驻</span>':''}</div><button class="more" data-edit>•••</button></div><h3>${esc(x.title)}</h3><p>${esc(x.note||'给它补充一句为什么重要吧。')}</p><footer><span>期限 ${fmtDate(x.date)}</span><div class="tiny-progress"><i style="width:${p}%"></i></div><button class="check ${x.status==='done'?'checked':''}" data-toggle-item>✓</button></footer></article>`}
function habitManageCard(h){const enabled=h.enabled!==false,done=h.history.includes(today);return `<article class="goal-card habit-manage-card ${enabled?'':'habit-hidden'}" data-habit-card="${h.id}"><div class="goal-top"><div><span class="goal-type">${h.emoji} 习惯</span>${enabled?'':'<span class="hidden-badge">已隐藏</span>'}</div><details class="card-menu"><summary aria-label="管理习惯">•••</summary><div><button data-habit-active="${h.id}">${enabled?'◌ 隐藏':'＋ 加入打卡'}</button><button data-edit-habit="${h.id}">✎ 编辑</button><button class="danger" data-delete-habit="${h.id}">× 删除</button></div></details></div><h3>${esc(h.title)}</h3><p>${enabled?`累计打卡 ${h.history.length} 天 · ${fmtDate(h.startDate) }开始`:'已从打卡指标和桌面模块中隐藏'}</p><footer>${enabled?`<button class="habit-card ${done?'checked':''}" data-habit="${h.id}"><strong>${done?'今日已完成 ✓':'完成今日打卡'}</strong></button>`:'<span class="hidden-habit-note">重新加入后才会计入打卡</span>'}</footer></article>`}
function renderHabitPage(){const habits=activeHabits(),done=habits.filter(h=>h.history.includes(today)).length;$('#managerContent').innerHTML=`<div class="welcome"><article class="welcome-card"><span>DAILY RHYTHM</span><h2>照顾身体，也是计划的一部分。</h2><p>今天完成 ${done}/${habits.length} 项，已隐藏的习惯不会计入统计。</p></article><article class="week-score"><div><h3>今日打卡</h3><p>仅统计已加入的习惯</p></div><div class="big-ring" style="--progress:${habits.length?done/habits.length*100:0}%"><b>${done}/${habits.length}</b></div></article></div><div class="section-heading"><div><span>MY HABITS</span><h2>每天想照顾的小事</h2><p>${data.habits.length-habits.length} 项已隐藏，可随时重新加入</p></div><button class="round-add" data-add="habits">＋</button></div><div class="page-grid">${data.habits.map(habitManageCard).join('')||empty('还没有习惯')}</div>`;bindActions($('#managerContent'))}
...
function executionAnalysis(){
  const daily=data.items.today,weekly=data.items.week,habits=activeHabits(),habitDone=habits.filter(h=>h.history.includes(today)).length;
  const dailyDone=daily.filter(x=>x.status==='done').length,weekDone=weekly.filter(x=>x.status==='done').length;
  const dailyRate=daily.length?Math.round(dailyDone/daily.length*100):0,weekRate=weekly.length?Math.round(weekDone/weekly.length*100):0,habitRate=habits.length?Math.round(habitDone/habits.length*100):0;
  ...
  return{daily,weekly,habits,habitDone,dailyDone,weekDone,dailyRate,weekRate,habitRate,overdue,active,pending,score,summary};
}
...
function renderReportPage(){
  ...
  const habits=a.habits.length?a.habits.map(h=>{const checked=h.history.includes(today);return `<div class="doc-habit ${checked?'checked':''}"><b>${h.emoji}</b><span>${esc(h.title)}</span><i>${checked?'✓':'—'}</i></div>`}).join(''):'<p class="doc-empty">还没有已加入的习惯</p>';
...
function bindActions(root){
  ...
  root.querySelectorAll('[data-edit-habit]').forEach(b=>b.onclick=()=>openEditor('habits',b.dataset.editHabit));
  root.querySelectorAll('[data-habit-active]').forEach(b=>b.onclick=()=>{const h=data.habits.find(x=>x.id===b.dataset.habitActive);h.enabled=!h.enabled;save();toast(h.enabled?'已加入打卡':'已隐藏，不计入统计')});
  root.querySelectorAll('[data-delete-habit]').forEach(b=>b.onclick=()=>{data.habits=data.habits.filter(x=>x.id!==b.dataset.deleteHabit);save();toast('习惯已删除')});
...
function persistEditor(){
  ...
  if(type==='habits'){
    const values={title,startDate:$('#editorDate').value||today,enabled:$('#editorStatus').value!=='hidden'};
    if(id){const habit=data.habits.find(x=>x.id===id);if(habit)Object.assign(habit,values)}else{id=uid();data.habits.push({id,...values,emoji:'🌱',history:[]})}
  }else{
    const values={title,note:$('#editorNote').value.trim(),date:$('#editorDate').value||today,status:$('#editorStatus').value,type,pinned:$('#editorPinned').checked,updatedAt:Date.now()};
    if(id){const current=data.items[type]?.find(x=>x.id===id);if(current)Object.assign(current,values)}else{id=uid();data.items[type].push({...values,id,createdAt:Date.now()})}
  }
...
function openEditor(type,id=''){clearTimeout(editorSaveTimer);editorDirty=false;const isHabit=type==='habits',x=isHabit?data.habits.find(h=>h.id===id):data.items[type]?.find(i=>i.id===id);$('#editorForm').reset();$('#editorType').value=type;$('#editorId').value=id;$('#editorTitle').value=x?.title||'';$('#editorNote').value=x?.note||'';$('#editorDate').value=(isHabit?x?.startDate:x?.date)||today;$('#editorStatus').innerHTML=isHabit?'<option value="active">加入打卡</option><option value="hidden">隐藏</option>':'<option value="todo">待开始</option><option value="doing">进行中</option><option value="done">已完成</option>';$('#editorStatus').value=isHabit?(x?.enabled===false?'hidden':'active'):(x?.status||'todo');$('#editorPinned').checked=!isHabit&&x?.pinned===true;$('#editorPinField').classList.toggle('hidden',isHabit);$('#editorNoteField').classList.toggle('hidden',isHabit);$('#editorDateLabel').textContent=isHabit?'开始日期':'执行期限';$('#editorHeading').textContent=id?'调整这项内容':`添加${labels[type]||'内容'}`;$('#editorEyebrow').textContent=isHabit?'日常节奏':periodLabel(type)||'NEW ITEM';$('#deleteItem').classList.toggle('hidden',!id);setAutoSaveState(id?'修改后自动保存':'填写标题后自动保存');$('#editorModal').classList.remove('hidden');setTimeout(()=>$('#editorTitle').focus(),40)}
...
function setup(){
  ...
  $('#editorForm').addEventListener('input',e=>{if(e.target.matches('#editorTitle,#editorNote,#editorDate,#editorStatus,#editorPinned'))scheduleEditorSave()});
  $('#editorForm').onsubmit=e=>{e.preventDefault();const wasNew=!$('#editorId').value;editorDirty=true;if(!persistEditor())return;closeEditor(false);toast(wasNew?'已添加并保存':'已保存')};
  $('#deleteItem').onclick=()=>{clearTimeout(editorSaveTimer);editorDirty=false;const type=$('#editorType').value,id=$('#editorId').value;if(type==='habits')data.habits=data.habits.filter(x=>x.id!==id);else data.items[type]=data.items[type].filter(x=>x.id!==id);closeEditor(false,false);save();toast('已删除')};
  ...
  $('#deepManage').onclick=e=>{e.preventDefault();location.href='rixu://manager'};
  window.addEventListener('storage',e=>{if(e.key===DB_KEY){data=load();if($('#editorModal').classList.contains('hidden'))renderAll();else renderWidget()}});
